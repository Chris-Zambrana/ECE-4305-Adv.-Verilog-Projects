        ...
        unique case({wr, rd})
            ...
            begin
                ...
                begin
                    ...
                    if ((rd_ptr_next == wr_ptr + 1)) // if the write pointer is one position before the read pointer, the buffer is full
                        full_next = 1'b1;
                    else 
                        full_next = 1'b0;
            ...
            begin
                if (~full)
                begin
                    wr_ptr_next = wr_ptr + 2; // move the write pointer by 2 since we are writing 2 values at a time
                    empty_next = 1'b0;
                    if ((wr_ptr_next == (rd_ptr - 1)) || (wr_ptr_next == rd_ptr)) // if the write pointer is one position before the read pointer, the buffer is full
                        full_next = 1'b1;
                end
            end
            ...
            begin
                ...
                begin
                    ...
                    if ((wr_ptr_next == (rd_ptr_next - 1)) || (wr_ptr_next == rd_ptr_next))
                        full_next = 1'b1;
                    else
                        full_next = 1'b0;
                end 
