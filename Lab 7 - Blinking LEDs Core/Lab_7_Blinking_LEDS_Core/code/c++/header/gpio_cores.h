...
/**
 * blinking LED core driver
 *  - write data into registers that determine the rate at which LEDs blink.
 *  - data being written is represented in ms
 *
 * MMIO subsystem HDL parameter:
 *  - W (not used in driver): # bits of output register
 *   (unused bits have no effect)
...
public:
    ...
    */
   enum {
      LED0_BLINK_REG = 0, /**< output data register */
      LED1_BLINK_REG = 1, /**< output data register */
      LED2_BLINK_REG = 2, /**< output data register */
      LED3_BLINK_REG = 3  /**< output data register */
   };
   /**
    * constructor.
    *
    ...
    *
    */
   void write_rate(int register_num, uint32_t data);

   // int read_rate(int register_num);
