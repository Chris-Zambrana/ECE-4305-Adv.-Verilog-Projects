...
(
   ...
   input  logic rx,
   output logic tx,      
   input logic rx_sensor,
   output logic tx_sensor,
   // 4 analog input pair
   ...
   // slot 14: Sensor UART 
   chu_uart_sensor #(.FIFO_DEPTH_BIT(8))  uart_slot14
   (.clk(clk),
    .reset(reset),
    .cs(cs_array[`S14_UART2]),
    .read(mem_rd_array[`S14_UART2]),
    .write(mem_wr_array[`S14_UART2]),
    .addr(reg_addr_array[`S14_UART2]),
    .rd_data(rd_data_array[`S14_UART2]),
    .wr_data(wr_data_array[`S14_UART2]), 
    .tx(tx_sensor),
    .rx(rx_sensor)
    );

   // slot 15: Servo PWM
    chu_io_pwm_servo #(.W(1), .R(21)) pwm_slot15 
    (.clk(clk),
     .reset(reset),
     .cs(cs_array[`S15_PWM2]),
     .read(mem_rd_array[`S15_PWM2]),
     .write(mem_wr_array[`S15_PWM2]),
     .addr(reg_addr_array[`S15_PWM2]),
     .rd_data(rd_data_array[`S15_PWM2]),
     .wr_data(wr_data_array[`S15_PWM2]),
     .pwm_out(pwm_servo)
     );
